...
public class ThirdPersonOrbitCamBasic : MonoBehaviour 
{
	...
	void Update()
	{
		...
		if (zeppelinScript != null) // Add this condition
		{
			container = zeppelinScript.GetAttachedContainer();
		}
	...
	bool ViewingPosCheck (Vector3 checkPos)
	{
		...
		// Use collisionLayers in SphereCast
		if (Physics.SphereCast(checkPos, 0.2f, direction, out RaycastHit hit, direction.magnitude, collisionLayers))
		{
			if (hit.transform != player && hit.transform != container && !hit.transform.GetComponent<Collider>().isTrigger)
			{
	...
	bool ReverseViewingPosCheck(Vector3 checkPos)
	{
		...
		// Use collisionLayers in SphereCast
		if (Physics.SphereCast(origin, 0.2f, direction, out RaycastHit hit, direction.magnitude, collisionLayers))
		{
			if (hit.transform != player && hit.transform != container && hit.transform != transform && !hit.transform.GetComponent<Collider>().isTrigger)
			{
				return false;
			}
		}
